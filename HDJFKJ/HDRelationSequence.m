...
@interface HDRelationSequence()

//用将需要排序的对象封装在HDRelationSequence对象中，包含排序的字符串，对象，首字母三个属性
//进行比较的字符串，
...
@implementation HDRelationSequence
...
+(NSMutableArray*)IndexArray:(NSArray*)stringArr{
    ...
    {
        NSString *pinyin = [((HDRelationSequence*)object).pinYin substringToIndex:1];
        //不同
...
+(NSMutableArray*)LetterSortArray:(NSArray*)stringArr{
    ...
    for (NSString* object in tempArray) {
        ...
        NSString *pinyin = [((HDRelationSequence*)object).pinYin substringToIndex:1];
        NSString *string = ((HDRelationSequence*)object).string;
        //不同
        if(![tempString isEqualToString:pinyin])
        {
            //分组
...
+(NSMutableArray*)ReturnSortChineseArrar:(NSArray*)stringArr{
    ...
    {
        HDRelationSequence *chineseString = [[HDRelationSequence alloc]init];
        ...
        chineseString.string = [HDRelationSequence RemoveSpecialCharacter:chineseString.string];
...
+(NSMutableArray*)sortObjectArray:(NSArray*)objectArray Key:(NSString *)key{
    ...
    NSString *tempString;
    //拼音分组
    for (HDRelationSequence* object in tempArray) {
    ...
    if (BMLog==1) {
        ...
        NSLog(@"HDRelationSequence user time in %f ms", useTime *1000.0);
    }
